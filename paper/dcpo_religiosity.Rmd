...
keywords: "xxxxx, religiosity, measurement"
...
surveys_rel <- read_csv(here::here("data-raw", "surveys_religiosity.csv"),
                       col_types = "cccccc") %>% 
    filter(str_detect(item, "attend"))

dcpo_input_raw <- DCPOtools::dcpo_setup(vars = surveys_rel,
                                        datapath = here::here("..",
                                                              "data", "dcpo_surveys"),
                                        file = here::here("data",
                                                          "dcpo_input_raw.csv"))
...
```{r dcpo_results, cache=TRUE, eval=FALSE}
...
```{r dcpo_results_summary, cache=TRUE, eval=FALSE}
...
```{r cs_plot, fig.cap="TCS Scores, Most Recent Available Year \\label{cs_mry}", fig.height=10, fig.width=8, eval=FALSE}
...
```{r ts_plots, fig.cap="TCS Scores Over Time Within Selected Countries \\label{ts}", fig.height=3.5, eval=FALSE}
...
```{r tcs_internal_val, fig.height = 4, fig.width= 8.5, fig.cap = "Internal Validation for Trust Civil Servants. \\label{tcs_iv}", eval=FALSE}
...
```{r construct_validation_survey, cache = TRUE, cache.extra = tools::md5sum(here::here("data", "tcs.rda")), fig.cap="Construct Validation: Correlations Between TCS Scores and Individual Trust in Institutions Survey Items \\label{surv_const_plot}", fig.height=4, eval=FALSE}
...
tcs_de_label <- tibble(year = 2000, mean = .8, label = tcs_de_cor)
...
tnatgov_plot + tcourt_plot + tcs_de_plot +
  plot_annotation(caption = "Note: Gray whiskers and shading represent 80% credible intervals.")
```

```{r construct_val, fig.height = 4,fig.width= 8.5, fig.cap = "Construct Validation for Trust Civil Servants . \\label{tcs_ev}", eval=FALSE}
...
tcs_cor_plot <- ggplot(tcs_corruption,
  ...
  ggtitle("Eurobarometer 88.2,CSES5, WVS7")  +
  geom_label(data = tcs_cor_label, aes(label = label))

#tcs_cor_plot


###voter_efficacy_time
 anes16_eff <- read.csv(here::here("data","anes16_eff.csv"))
 
 us_int_eff <-  anes16_eff %>%   
   select(year, int_eff,n) %>%
   filter(int_eff != 0) %>%
   mutate(country = "United States") %>%
   mutate( prop = int_eff/n,
           se = sqrt((prop*(1-prop))/n),
           prop_90 = prop + qnorm(0.90)*se,
           prop_10 = prop - qnorm(0.90)*se) %>%
   left_join(theta_results %>% 
               select(-kk, -tt) %>%
               filter(country == "United States"),
             by = c("country", "year")) %>%
 ...
 inteff_us_cor <- map(tcs_inteff_us, function(x) 
   with(x, cor(tcs_mean, var))) %>%
   unlist() %>% 
   mean() %>% 
   round(2) %>% 
   paste0("R = ", .)  #0.52 
 ...
 inteff_us_label <- tibble(year = 2005, mean = .8, label = inteff_us_cor)
 inteff_us_plot <- us_int_eff %>% 
   ggplot(aes(x = year,
              y = mean)) +
   geom_line() +
   geom_ribbon(aes(ymin = q10,         
   ...
   geom_segment(aes(x = year, xend = year,
                ...
                na.rm = TRUE,
                alpha = .2) +
   annotate("text", x = 1984, y = .72, size = 3,
            label = 'ANES') +
   annotate("text", x = 1982, y = .36, size = 3,
            label = "Trus Civil Servants") +
   labs(x = "Year", 
        y = "Scores") +
   theme_bw() +
   theme(legend.position="none",
   ...
   geom_label(data = inteff_us_label, aes(label = label)) #, size = 3
...
tcs_sat_label <- tibble(mean = 0.7, prop = 75, label = tcs_sat_cor)
...
tcs_sat_plot <- ggplot(sat_tcs,
  ...
  theme_bw() + theme(legend.position="none",
                     ...
                     axis.title = element_text(size=9)) +
  coord_cartesian(xlim=c(0, 1)) +
  labs(x = "Trust Civil Servants", y = "% satisfying with the political system performance,\n WVS7") +
  ggtitle("WVS7")  +
  geom_label(data = tcs_sat_label, aes(label = label))

#tcs_sat_plot

 
tcs_cor_plot + inteff_us_plot + tcs_sat_plot + patchwork::plot_annotation(caption = "Note: Gray whiskers and shading represent 80% credible intervals.")
